...
mean.hour.depthavg <- mean.hour %>% filter(depth != 25, depth != 15, depths == 5, heatStat == "U") %>% group_by(date, heater, block, time_hour) %>% summarize(UnheatedMean = mean(temp)) %>% left_join(mean.hour %>% filter(depth != 25, depth != 15, depths == 5, heatStat == "H") %>% group_by(date, heater, block, time_hour) %>% summarize(HeatedMean = mean(temp))) %>% mutate(TempDiff = HeatedMean - UnheatedMean) %>% full_join(mean.hour.depthavg %>% filter(heater %in% c("ON", "ON*")) %>% group_by(time_hour) %>% summarize(TempDiff = mean(TempDiff)) %>% mutate(heater = "ON", block = "all"))
...
```{r}
mean.4hour.blockavg <- mean.4hour %>% filter(heater %in% c("ON", "ON*") & !depth %in% c(15, 25)) %>% spread(key = heatStat, value = temp) %>% mutate(TempDiff = H - U) 

###Why are there instances on 8/19/21 where the date and time list different dates???

%>% group_by(time_4hour, depth) %>% summarize(TempDiff = mean(TempDiff, na.rm = T), sd = sd(TempDiff, na.rm = T), n = length(!is.na(depth)))
```