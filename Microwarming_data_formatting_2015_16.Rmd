...
unknown <- long %>% filter(block == "unknown")
```
...
long <- long %>% mutate(flag = if_else(value >= 20 & value <= -5, "outlier", "NA"))
...
for(i in 1:length(combinations$block)) {
  ...
  early <- long %>% filter(block == plot, depth == cm, type == MC, heatStat == HU, regStat == RM, dist == distance)
...
#flag block 2 Ctl as problematic
flagged <- flagged %>% mutate(flag = if_else(block == 2 & date >= "2015-10-14" & type == "Ctl", "mislabeled", flag))
```
