...
#In order to select a 4-hour rolling window, gap-fill the dataset so that is a row for every minute.  (There are many missing values in the dataset.  This adds NA rows for these missing values, with an appropriate time stamp.)  This is more efficient than searching for a date range with myrollapply (as in myrollapply.2). 
