...
####For testing code; remove this chunk after
```{r}
long <- long %>% filter(date > "2016-05-14" & date <= "2016-05-15" | date > "2016-06-16" & date <= "2016-06-17")
```
...
#Omit gap-filled timestamps and one-off value (no other values within the 1-hour window)
flagged.early <- flagged.early %>% unique() %>% filter(!is.na(value), diff != 0)
...
#Omit gap-filled timestamps and one-off value (no other values within the 1-hour window)
flagged.late <- flagged.late %>% unique() %>% filter(!is.na(value), diff != 0)
...
outliers <- testframe %>% filter(block == "1", heatStat == "U", )
