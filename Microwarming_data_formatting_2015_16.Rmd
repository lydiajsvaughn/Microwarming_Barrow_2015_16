...
long$time_15min <- floor_datetime(long$time, 15 * 60)

#calculate the average across all depths for evey 15-minute interval
long.15min.avg <- long %>% group_by(block, type, time_15min) %>% summarize(UAvg_15min = mean(UAvg), HAvg_15min = mean(HAvg)) %>% mutate(TempDiff_15min = HAvg_15min - UAvg_15min)

#calculate the average for each depth and each 15-minute interval
long.15min <- long %>% group_by(block, type, time_15min, heatStat, depth) %>% summarize(temp = mean(value)) 
...
```{r}
#flaggedBarrowFile_2016.csv (the original dataset plus outlier flags)

#longBarrow.15min.avg_2016.csv (temperatures averaged across all depths for every 15-min interval)

#longBarrow.15min_2016.csv (temperatures averaged for each depth for every 15-min interval)
