...
for(i in 2:length(switch1$heater)) {
  ...
  #switch1 <- switch1 %>% mutate(heatSwitch[i] = ifelse(switch1$heater[i] %in% c("ON", "ON*") & !(switch1$heater[i-1] %in% c("ON", "ON*")), "heat.on", NA))
  
  switch1[i,]$heatSwitch <- ifelse(switch1$heater[i] %in% c("ON", "ON*") & !(switch1$heater[i-1] %in% c("ON", "ON*")), "heat.on", NA)
