...
```{r}
flagged.2015 <- read.csv('data/2015_organized/flaggedBarrowFile_2015_4.csv', stringsAsFactors=F) #Data following QA/QC, with column flagging outliers
...
```{r}
flagged.2016 <- read.csv('data/2016_organized/flaggedBarrowFile_2016_7.csv', stringsAsFactors=F) #Data following QA/QC, with column flagging outliers
...
####For each flux measurement, calculate the corresponding mean temperatures for the 2-hour window leading up to that measurement time.  
```{r}
...
flux.temp <- bind_rows(flux.2015, flux.2016) %>% mutate(temp_mean_10_20_35 = (temp_10cm + temp_20cm + temp_35cm) / 3)

write.csv(write.csv(flux.temp, file = "data/flux.temp.csv"))
```