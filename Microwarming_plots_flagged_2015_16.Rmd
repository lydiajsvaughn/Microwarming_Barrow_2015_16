...
#This depth-averaged dataframe needs work
...
ggsave("flagged_TempDiff_2016_4.png", path="plots/outliers_omitted", width = 14, height = 7, dpi = 300)

#Control boards only
# p1.ctl <- ggplot(mean.15min.depthavg %>% filter(type == "Ctl") %>% filter(block %in% c(1,2,3,4)) %>% filter(TempDiff<=20 & TempDiff>=-10), aes(y=TempDiff, x=time_15min, color=as.factor(as.character(block)))) +
#   geom_point(pch=1) +
#   ylab(expression("Heated minus control"~(degree*C))) +
#   xlab("Time") +
#   scale_color_discrete(name="Block") +
...
# 
# #ggsave("TempDiff_2015.png", path="plots", width = 14, height = 7, dpi = 300)
# ggsave("flagged_TempDiff_2016_ctl.png", path="plots/outliers_omitted", width = 14, height = 7, dpi = 300)
```

####Plot 2: depth-averaged temperature in heated and unheated blocks, using the regulator board output
```{r}
...
#Including both control and monitor boards
p2 <- ggplot(heatstat.15min.avg %>% filter(block %in% c(1,2,3,4)), aes(y = temp_15min, x = time_15min, color = Heatstat, pch = block)) +
   geom_point() +
...
ggsave("flagged_UavgHavg_15min_2016_4.png", path="plots/outliers_omitted", width = 14, height = 7, dpi = 300)
...
for(i in 1:length(unique(chambers$block))){
  ...
  p3 <- ggplot(mean.15min %>% filter(block == plot), aes(y = temp, x = time_15min, color = heatStat)) +
  geom_point(pch = 1) +
  facet_grid(depth~.) +
  ylab(expression("Temperature" ~ (degree * C))) +
...
ggsave(file = paste("flagged_", plot, "_15min_tempdepth_2016_4.png", sep=""), path="plots/outliers_omitted", width = 8.5, height = 11, dpi = 300)
...
ggsave(file = paste("flagged_", plot, "_15min_tempdepth_2016_ctl_4.png", sep=""), path="plots/outliers_omitted", width = 8.5, height = 11, dpi = 300)

}

```

####Plot 4: Temperature in heated and unheated plots within each block at each depth, using 1-hour averaged data
You can see that heating effects vary widely among different blocks and depths. 
```{r}
#Including both control and monitor boards
chambers <- mean.hour 
...
for(i in 1:length(unique(chambers$block))){
  ...
  p4 <- ggplot(mean.hour %>% filter(block == plot), aes(y = temp, x = time_hour, color = heatStat)) +
...
ggsave(file = paste("flagged_", plot, "_1h_tempdepth_2016_4.png", sep=""), path="plots/outliers_omitted", width = 8.5, height = 11, dpi = 300)
...
ggsave(file = paste("flagged_", plot, "_1h_probe_tempdepth_2016_4.png", sep=""), path="plots/outliers_omitted", width = 16, height = 11, dpi = 300)
...
ggsave(file = paste("flagged_", plot, "_15min_probe_tempdepth_2016_4.png", sep=""), path="plots/outliers_omitted", width = 16, height = 11, dpi = 300)
