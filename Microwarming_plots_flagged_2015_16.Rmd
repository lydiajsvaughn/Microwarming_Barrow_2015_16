...
p1 <- ggplot(mean.15min.depthavg %>% filter(block %in% c(1,2,3,4)) %>% filter(TempDiff<=20 & TempDiff>=-10), aes(y=TempDiff, x=time_15min, color=as.factor(as.character(block)))) +
  geom_point(pch=1) +
...
ggsave(file = paste("flagged_", plot, "_tempdepth_2016_4.png", sep=""), path="plots/outliers_omitted", width = 8.5, height = 11, dpi = 300)
